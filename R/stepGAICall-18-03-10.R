                                  ...
                                  mu.try=TRUE, sigma.try=TRUE, nu.try=TRUE, tau.try=TRUE, ...)
{
    ...
    assign("objectAll", object, envir = .GlobalEnv)
    ...
    cat("---------------------------------------------------", "\n")
     if ("mu" %in% object$par && mu.try==TRUE)
         {
     current.par <- "mu"
         iferror <- try( assign("objectAll", gamlss:::stepGAIC.VR(objectAll, scope=scope, direction="forward", what = "mu", ...), 
    ...
    #-------------------- 
     if ("sigma" %in% object$par && sigma.try==TRUE)
        ...
        iferror <- try( assign("objectAll", gamlss:::stepGAIC.VR(objectAll, scope=sigma.scope, direction="forward", 
                       what = "sigma", ...), envir = .GlobalEnv)  , silent = TRUE) 
             if (any(class(iferror)%in%"try-error"))
                 {
                  cat("---------------------------------------------------", "\n") 
     ...
     if (  "nu" %in% object$par && nu.try==TRUE)
        ...
        iferror <- try( assign("objectAll", obj<-gamlss:::stepGAIC.VR(objectAll, scope=nu.scope, direction="forward", 
                         what = "nu", ...), envir = .GlobalEnv), silent = TRUE)           
             if (any(class(iferror)%in%"try-error"))
                 { 
                    cat("---------------------------------------------------", "\n")
     ...
     if ( "tau" %in% object$par && tau.try==TRUE)
        ...
        iferror <- try( assign("objectAll", gamlss:::stepGAIC.VR(objectAll, scope=tau.scope, direction="forward", 
                       what = "tau", ...), envir = .GlobalEnv), silent = TRUE) 
             if (any(class(iferror)%in%"try-error"))
                 { 
                   cat("---------------------------------------------------", "\n")
     ...
     #------------------ 
      ...
      iferror <- try( assign("objectAll", gamlss:::stepGAIC.VR(objectAll, scope=scope, direction="backward", 
                     what = "mu", ...)  , envir = .GlobalEnv)  , silent = TRUE) 
             if (any(class(iferror)%in%"try-error"))
                 { 
                  cat("---------------------------------------------------", "\n") 
